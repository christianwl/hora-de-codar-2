...
function ArmazenarMaiorNumero(quantidade, opcao) {
  ...
  do {
    var listaLocal = prompt(
      `Digite o º${indice + 1} número: \n\nOBS: Digite um número diferente dos anteriores!!`,
    );
    ...
    if (isNaN(listaLocal)) {
      ...
      alert(
        `O número "${listaLocal}" é igual ao º${indice + 1} número digitado! Por favor digite outro valor`,
      );
    } else {
  ...
  switch (opcao) {
    case 1:
      ...
      break;
    case 2:
      segundoMaiorNumero = VerificarMaiorNumero(
        listaNumeros.filter((value) => value < maiorNumero),
      );
      ApresentarNaTela(
        `A soma dos dois maiores números, ${maiorNumero} e ${segundoMaiorNumero} é: ${maiorNumero + segundoMaiorNumero}`,
      );
      break;
    case 3:
      ApresentarNaTela(
        `O primeiro número é ${listaNumeros[0]}, o ultimo número é ${listaNumeros[listaNumeros.length - 1]}, e o maior número é ${maiorNumero}.`,
      );
      break;
    default:
      ApresentarValorIncorreto();
...
function Calculadora(quantidade) {
  ...
  for (var i = 0; i < quantidade; i++) {
    valores.push(
      parseInt(prompt(`Digite o º${i + 1} número a ser calculado: `)),
    );
  }

  do {
    tiposDeCalculo =
      prompt(`Digite o número que corresponda ao calculo que deseja: 
        \n\n[ 1 ] Adição
        \n\n[ 2 ] Subtração
        \n\n[ 3 ] Divisão
        \n\n[ 4 ] Multiplicação`);
  ...
  switch (parseInt(tiposDeCalculo)) {
    case 1:
      ApresentarNaTela(
        `O resultado da conta é: ${CalcularResultado(valores, "+")}`,
      );
      break;
    case 2:
      ApresentarNaTela(
